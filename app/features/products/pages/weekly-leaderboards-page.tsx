...
export const meta: Route.MetaFunction = ({ params }) => {
  ...
  // Format dates manually to avoid Intl locale issues
  ...
  return [
    {
      title: `The best of week ${formatDateShort(weekStart)} - ${formatDateShort(weekEnd)} | wemake`,
    },
  ];
};
...
export default function WeeklyLeaderboardsPage({
    ...
    const isToday = urlDate.equals(DateTime.now().setZone("Asia/Seoul").startOf("week"));

    return (
      <div className="container mx-auto px-4 py-8">
        <PageHeader title={`The best of week ${formatDateShort(urlDate.startOf("week"))} - ${formatDateShort(urlDate.endOf("week"))}`} />
        <div className="flex justify-center items-center gap-2">
          <Button variant="outline" asChild>
            <Link
              to={`/products/leaderboards/weekly/${previousWeek.year}/${previousWeek.weekNumber}`}
            >
              &larr; {formatDateShort(previousWeek)}
            </Link>
          </Button>
